...
// Please visit https://github.com/miloyip/rapidjson/tree/master/example
// for more examples
...
int main(int, char* []) {
  ...
  assert(document.IsObject());  // Document is a JSON value represents the root
                                // of DOM. Root can be either an object or
                                // array.
  ...
  // Adding values to array.
  {
    ...
    for (int i = 5; i <= 10; i++)
      a.PushBack(i, allocator);  // May look a bit strange, allocator is needed
                                 // for potentially realloc. We normally uses
                                 // the document's.
  ...
  assert(author.IsNull());  // Move semantic for assignment. After this variable
                            // is assigned as a member, the variable becomes
                            // null.
