...
#TODO configure python include
if [ -d "../com_argcv" ]; then
  script_path="../com_argcv"
else
  script_path=$(dirname $(dirname $(dirname "$0")))
  script_path=${script_path:-.}
fi

[ -f "$script_path/util/utils.sh" ] && . "$script_path/util/utils.sh"
  ...
  if [ -z "$PYTHON_LIB_PATH" ]; then
    ...
    if [ 1 = "$USE_DEFAULT_PYTHON_LIB_PATH" ]; then
      ...
      if is_non_interactive_mode; then
        b=""
        echo "non-interactive mode, skipped..."
      else
        read b || true
      fi
...
# to create junctions on Windows.
  ...
  sed -e "s/\$PYTHON_MAJOR_VERSION/$python_major_version/g" \
      ...
      tools/bazel.rc.template >> $BAZELRC
